...
class AbstractTunnel(StartableObject.ScenarioState):
    """
    Attributes
    ----------
    ...
    def get_tunnel_id(self):
        return NetworkBackendUtil.szudzik_pairing_function(self.emulation_node_x._node._id, self.emulation_node_y._node._id)
    ...
    def get_local_emulation_node(self):
        return self.emulation_node_x if singletons.simulation_manager.is_local_node(
            self.emulation_node_x._node._id) else self.emulation_node_y
...
class TunnelIPRoute(AbstractTunnel):
    ...
    def get_tunnel_name(self):
        return singletons.network_backend.get_tunnel_name(self.emulation_node_x._node._id, self.emulation_node_y._node._id)
...
class GreTapTunnel(TunnelIPRoute):
    def _start(self):
        ...
        self._logger.info("creating gretap tunnel for %s<->%s" % (self.emulation_node_x._node._id, self.emulation_node_y._node._id))

        self.add_command(self.EVENT_TUNNEL_ADD, tunnel_cmd)

        self._logger.info("changing NIC MTU to '%d' ... ", GRETAP_MTU)
