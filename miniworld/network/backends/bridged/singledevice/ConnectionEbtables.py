...
from miniworld.network.backends.bridged.Connection import ConnectionDummy
...
def ConnectionEbtables():
    class ConnectionEbtables(ConnectionDummy()):
        ...
        def set_ebtables_forward_policy(self, policy):
            self._logger.info("{policy} all packets in FORWARD chain ...".format(policy=policy))
        ...
        def get_ebtables_atomix_prefix():
            return "--atomic-file %s" % ConnectionEbtables.atomic_file if singletons.scenario_config.is_network_backend_bridged_execution_mode_batch() else ""
