...
class GlobalConfig(JSONConfig.JSONConfig):
    ...
    def get_log_level(self):
        ...
        log_level_str = _get_log_level(self).upper()
...
def set_global_config(*args, **kwargs):
    """ Set the global singletons.config.
    ...
    # we have not configured logging at this point
    print("setting global config file '%s'" % args[0], file=sys.stderr)
    singletons.config = GlobalConfig()
    singletons.config.data = deepcopy(_config)
    return _config
