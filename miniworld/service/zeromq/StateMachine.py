    ...
    def expect(self):
        ...
        if len(msgs) == expected_msg_parts:
            ...
            if state == self.state:
                ...
                self._logger.critical("client requested state '%s', but current state is '%s'", state, self.state)
        else:
            self._logger.critical("Received invalid message: %s, expected %d parts in the message, has: %d", msgs, expected_msg_parts, len(msgs))
