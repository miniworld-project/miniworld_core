...
class NetworkManager(ResetableInterface, NetworkBackendNotifications.NetworkBackendNotifications):
    ...
    def before_distance_matrix_changed(self, simulation_manager, network_backend, changed_distance_matrix,
        ...
        for (x, y), distance in full_distance_matrix.items():
            connected, _ = simulation_manager.impairment.distance_2_link_quality(distance)
            if connected:
    ...
    # TODO:
    def get_network_graph(self, include_management_node=False, include_nodes=True):
        """
        ...
        # self._logger.info("topology: %s", pformat(G.edges()))
        return d
