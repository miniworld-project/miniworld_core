...
def NetworkBackendBridgedSingleDevice():
    ...
    class NetworkBackendBridgedSingleDevice(NetworkBackendBridgedDummy()):
        ...
        EBTABLES_EVENT_ORDER = OrderedSet([EVENT_EBTABLES_INIT, EVENT_EBTABLES_CREATE_CHAINS, EVENT_EBTABLES_REDIRECT, EVENT_EBTABLES_COMMANDS, EVENT_EBTABLES_FINISH])
        ...
        def add_shell_ebtables_command(event, cmd):
            singletons.network_backend.shell_command_executor.add_command(NetworkBackendBridgedSingleDevice.EBTABLES_EVENT_ROOT, event,
        ...
        def before_simulation_step(self, simulation_manager, step_cnt, network_backend, emulation_nodes, **kwargs):
            ...
            if scenario_config.is_network_backend_bridged_execution_mode_batch():
                self.add_shell_ebtables_command(self.EVENT_EBTABLES_INIT, conn_type.ebtable_cmd_atomic_save)
        ...
        def before_link_initial_start(self, network_backend, emulation_node_x, emulation_node_y, interface_x, interface_y, connection_info,
            ...
            super(NetworkBackendBridgedSingleDevice, self).before_link_initial_start(network_backend, emulation_node_x, emulation_node_y, interface_x, interface_y, connection_info, start_activated=staticmethod, **kwargs)
            ...
            if not self.bridges.get(br_name, None):
                ...
                self.add_shell_ebtables_command(self.EVENT_EBTABLES_CREATE_CHAINS, connection_type.get_ebtables_chain_cmd(br_name, connection_type.policy_drop))
                # redirect to new chain
                self.add_shell_ebtables_command(self.EVENT_EBTABLES_REDIRECT, connection_type.get_ebtables_redirect_cmd(br_name))
            ...
            # TODO: nearly same code as in NetworkBackendBridgedMultiDevice!
                ...
                if connection_info.is_remote_conn:
                    ...
                    remote_node, if_remote_node, local_emu_node, if_local_emu_node = singletons.simulation_manager.get_remote_node(emulation_node_x, emulation_node_y, interface_x, interface_y)
